...
export async function runPipeline(topic = null, category = null, fbPageId = null) {
  ...
  try {
    ...
    //   - Có reference photo → Sharp + SVG overlay (giữ NGUYÊN ảnh thật, đè text/badge lên)
    //   - Không có reference → AI generate (gpt-image-2 from scratch)
    ...
    try {
      if (sourceMedia) {
        // Có ảnh thật → Sharp overlay (giữ nguyên 100% pixel ảnh tham chiếu)
        ...
        console.log(`[Pipeline] Image composed via Sharp (preserves reference): ${finalImage.path}`);
      } else {
        // Không có ảnh thật → AI generate
        ...
        console.log(`[Pipeline] Image generated by AI (no reference): ${finalImage.path}`);
      }
    } catch (err) {
      console.error(`[Pipeline] Compose failed:`, err.message);
